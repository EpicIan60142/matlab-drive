%% ASEN 6014 Coursera Relative Motion Control Quiz 3 Main
...
tspan = [0:10:4840, 4848]; %4850:10:6000];
    ...
    % Run controller
        % No disturbance
distConst.aDragCoeff = 0;
[t, X] = ode45(@(t,X)inertialFeedbackControlCoeffs(t,X,coeff,kConst,pConst,distConst), tspan, X0, opt);

        % Unmodeled disturbance
distConst.aDragCoeff = aDragCoeff;
[t_dist, X_dist] = ode45(@(t,X)inertialFeedbackControlCoeffs(t,X,coeff,kConst,pConst,distConst), tspan, X0, opt);
...
figure;
hold on; grid on;
title("Inertial Relative Motion Control - undisturbed")
...
%% Plot and report disturbed results
    % Extract states
X_c = X_dist(:,1:6);
X_d = X_dist(:,7:12);
...
figure;
hold on; grid on;
title("Inertial Relative Motion Control - disturbed")
scatter3(X_d_Hill(1,1), X_d_Hill(1,2), X_d_Hill(1,3), markerSize, 'g', 'filled');
plot3(X_d_Hill(:,1), X_d_Hill(:,2), X_d_Hill(:,3), 'b-')
scatter3(X_d_Hill(end,1), X_d_Hill(end,2), X_d_Hill(end,3), markerSize, 'r', 'filled')
% scatter3(0,0,0,markerSize,'k','*')
