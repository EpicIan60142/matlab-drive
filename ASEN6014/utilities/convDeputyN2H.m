function x_d_H = convDeputyN2H(x_c_N, x_d_N, const)
% Function that converts a deputy spacecraft state from inertial
% coordinates to Hill frame coordinates
%   Inputs:
%       - x_c_N: Chief spacecraft state in inertial coordinates as follows:
%                x_c_N = [r_c_N; v_c_N]
%       - x_d_N: Deputy spacecraft state in inertial coordinates as
%                follows:
%                x_d_N = [r_d_N; v_d_N]
...
% Map vectors
rho_d_H = HN*(r_d_N - r_c_N);
rhoPrime_d_H = HN*(v_d_N - v_c_N) - cross(w_ON_H, rho_d_H);
