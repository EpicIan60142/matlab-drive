...
%   Inputs:
%       - X: Full state history of the problem, containing both spacecraft
%            and station states at every measurement time. Organized as
%            [X_1, X_2, ..., X_tMeas]
...
[tMeas, ~, ~, ~] = processStations(stations);
tMeas = tMeas(2:end);
...
for k = 1:dTime:length(tMeas)
    ...
    nexttile([2 1]) % Make 3d plot span 2 rows and 1 column
        ...
        for kk = 1:length(stations)
            plot3(X_stat(3*kk-2,k), X_stat(3*kk-1,k), X_stat(3*kk,k), 'Color', stations(kk).color, 'Marker', '.', 'MarkerSize', 15);
        end
