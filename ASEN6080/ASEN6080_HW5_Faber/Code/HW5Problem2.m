...
Q0_LKF = diag(sigAccel^2*ones(1,3));
% Q0_SRIF = diag((10*sigAccel)^2*ones(1,3));
Q0_SRIF = Q0_LKF;

SRIFRun = runSRIF(X0, x0, P0, Q0_SRIF, uBar, true, earthConst, stations, X_ref, t_ref, 1, true);
LKFRun = runLKF_SNC(X0, x0, P0, Q0_LKF, earthConst, stations, X_ref, t_ref, 1, true);

    % Plot difference between SRIF and LKF
titleText = sprintf("SRIF vs. LKF State Difference - w/ process noise (X_{SRIF} - X_{LKF})");
