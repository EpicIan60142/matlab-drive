...
for iterations = 1:1 %options.num_iters
    for j = 1:length(obs.times(:))
        ...
        %  Time Update with SNC
        xbar = Phi*x_hat;
        Pbar = Phi*Covariance*Phi';

        C = feval(options.obs_fcn, obs, X_Int(end,1:6), j, Time_Int(end), options.extra_args );
        OminusC = (obs.obs(j,:)-C)';
        residuals(j,:) = OminusC;
        ...
        if ((j >= options.num_obs_switch) && (obs.station(j) == obs.station(j-1)))
