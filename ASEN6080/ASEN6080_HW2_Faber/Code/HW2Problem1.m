...
if false
    generateTruthData_MuJ2(earthConst, orbital, x0Perturb, stations, filename, numOrbits, dt);
end

load("HW2Problem1Data.mat");
...
batchRun = runBatch(X0, x0, P0, earthConst, stations, X_ref, tspan, opt);

%% Problem 1b: CKF/LKF
fprintf("\n1b. LKF\n\n")

LKFRun = runLKF(X0, x0, P0, earthConst, stations, X_ref, t_ref);
...
EKFRun = runEKF(X0_EKF, P0_EKF, earthConst, stations, X_ref, t_ref, t_EKF_start);
...
batchRun_d = runBatch(X0, x0_d, P0, earthConst, stations, X_ref, tspan, opt);

LKFRun_d = runLKF(X0, x0_d, P0, earthConst, stations, X_ref, t_ref);
...
batchRun_e = runBatch(X0, x0_d, P0_e, earthConst, stations, X_ref, tspan, opt);

LKFRun_e = runLKF(X0, x0_d, P0_e, earthConst, stations, X_ref, t_ref);
...
batchRun_f = runBatch(X0, x0, P0, earthConst, stations_f, X_ref, tspan, opt);

LKFRun_f = runLKF(X0, x0, P0, earthConst, stations_f, X_ref, t_ref);

numMeas = 157;
X0_EKF = LKFRun_f.X_LKF(:,numMeas);
P0_EKF = LKFRun_f.LKFOut.PEst{numMeas};
t_EKF_start = LKFRun_f.t_LKF(numMeas);
EKFRun_f = runEKF(X0_EKF, P0_EKF, earthConst, stations_f, X_ref, t_ref, t_EKF_start);
