...
avgThrust = []; % Initialize as an empty array
...
%% Confidence Interval
SEM_ISP = stdISP/sqrt(size(isp,1));

SEM_CI_95(1) = avgISP - 1.96*SEM_ISP;
SEM_CI_95(2) = avgISP + 1.96*SEM_ISP;

SEM_CI_975(1) = avgISP - 2.24*SEM_ISP;
SEM_CI_975(2) = avgISP + 2.24*SEM_ISP;

SEM_CI_99(1) = avgISP - 2.58*SEM_ISP;
SEM_CI_99(2) = avgISP + 2.58*SEM_ISP;

test_fire_1_95 = ceil(stdISP*1.96/0.1)^2;
test_fire_1_975 = ceil(stdISP*2.24/0.1)^2;
test_fire_1_99 = ceil(stdISP*2.58/0.1)^2;

test_fire_2_95 = ceil(stdISP*1.96/0.01)^2;
test_fire_2_975 = ceil(stdISP*2.24/0.01)^2;
test_fire_2_99 = ceil(stdISP*2.58/0.01)^2;

%% Plotting
for i = 1:numFiles
    f = figure;
    %f.Position = [720, 469, 1300,720];
    filename = ['ThrustPlot',num2str(i),'.png'];
    subplot(1,2,1)
    hold on;
    title("Raw Thrust Data")
    plot(files{i}.time, files{i}.thrust);
    plot(files{i}.time(1:end-4), avgThrust(:,i))
    xline(files{i}.time(start(i)));
    xline(files{i}.time(stop(i)));
    xlabel("Time (sec)")
    ylabel("Thrust (N)")
    legend("Raw Data", "5-value moving average")
    hold off;
