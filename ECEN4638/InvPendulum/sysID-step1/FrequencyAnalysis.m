...
H4 = H2./H3; % Check if SysID for H1 is right
...
% % Black Boxing H2
% model2 = 0.0356 * s;          %zero at origin
% model2 = (45.528 * s)/(s+25.7611)^2;    % first double pole, undamp
% model2 = (0.036* s)/(s^2/25.7611^2 + s/(1.39*25.7611) + 1);   % first double pole w/ damping
% model2 = (0.0356* s)/((s/41.47+1)*(s^2/21.363^2 + s/(1.39*21.363) + 1)); % Added extra pole
model2 = 0.0356*(s*(s^2/95.504^2 + s/(114.6687 * 95.504) + 1))/((s/41.47+1)*(s^2/21.363^2 + s/(1.39*21.363) + 1) * (s^2/126.92^2 + s/(12.1983 * 126.92) + 1)); % Final TF for H2
...
% Black Boxing H3
kh3 = 0.008;
model3 = (kh3*s^2) * ((s^2/95.504^2 + s/(95.504 * 114.6687) + 1)) / ((s^2/18.2212^2 + s/(18.2212 * 9.927) + 1) * (s^2/126.92^2 + s/(126.92 * 30.865) + 1));
...
hold on
title("Amplitude Bode Plot of H1 (u to Theta 1)")
...
hold on
title("Phase Bode Plot of H1 (u to Theta 1)")
...
% semilogx(w(w<=N*wr), db(abs(H4(w<=N*wr)))) % Experiment
hold on
title("Amplitude Bode Plot of H2 (u to strainOut)")
...
subplot(2,1,2)
semilogx(w(w<=N*wr), rad2deg(unwrap(angle(H2(w<=N*wr))))) % Experiment
% semilogx(w(w<=N*wr), rad2deg(unwrap(angle(H4(w<=N*wr))))) % Experiment
hold on
title("Phase Bode Plot of H2 (u to strainOut)")
...
% semilogx(w(w<=N*wr), db(abs(H4(w<=N*wr)))) % Experiment
hold on
title("Amplitude Bode Plot of H3 (theta 1 to strainOut)")
...
subplot(2,1,2)
semilogx(w(w<=N*wr), rad2deg(unwrap(angle(H3(w<=N*wr))))) % Experiment
% semilogx(w(w<=N*wr), rad2deg(unwrap(angle(H4(w<=N*wr))))) % Experiment
hold on
title("Phase Bode Plot of H3 (theta 1 to strainOut)")
...
model2new = 0.0356*(s) / ((s/41.47+1)*(s^2/21.363^2 + s/(1.39*21.363) + 1));
model3new = kh3*(s^2) / ((s^2/18.2212^2 + s/(18.2212 * 9.927) + 1));

model1new = model2new/model3new;

T = [model1new; model2new; s.*model1new; s.*model2new;];
oldSys = ss(T,'minimal') 
