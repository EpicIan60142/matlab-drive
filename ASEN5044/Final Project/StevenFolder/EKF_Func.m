...
for k = 1:(length(y_actual) - 1)
    ...
    for i = 1:vec_size
        ...
        rho_i = sqrt((X - Xs_t(k))^2 + (Y - Ys_t(k))^2);
    
        %Measurement Derivatives
        dH1_dX1 = (X - Xs_t(k))/rho_i;
        dH1_dX3 = (Y - Ys_t(k))/rho_i;
        dH2_dX1 = (Y - Ys_t(k))*( (Y - Ys_t(k))*(Xdot - Xdots_t(k)) - (X - Xs_t(k))*(Ydot - Ydots_t(k)) )/(rho_i^3);
        dH2_dX2 = (X - Xs_t(k))/rho_i;
        dH2_dX3 = (X - Xs_t(k))*( (X - Xs_t(k))*(Ydot - Ydots_t(k)) - (Y - Ys_t(k))*(Xdot - Xdots_t(k)) )/(rho_i^3);
        dH2_dX4 = (Y - Ys_t(k))/rho_i;
        dH3_dX1 = -(Y - Ys_t(k))/(rho_i.^2);
        dH3_dX3 = (X - Xs_t(k))/(rho_i.^2);
    ...
    sigComp = [];
    for kk = 1:size(P_k_plus, 1)
        sigComp = [sigComp; sqrt(P_k_plus(kk,kk,k+1))];
    end
    sig_KF = [sig_KF, sigComp];
