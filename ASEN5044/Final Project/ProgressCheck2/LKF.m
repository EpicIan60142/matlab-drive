...
%   Author: Ian Faber, 12/09/2024
    ...
    % Account for matrix output from orbitMonteCarlo.m
if ~iscell(y)
    yCell = {};
    for k = 1:size(y,2)
        meas = [];
    ...
    % Initial constants
dT = filterInit.dt; % Important for Jacobians
...
dt = filterInit.dt; % Matches Jacobians
...
for k = 1:length(y)-1 % Go to length(y) - 1 because correction step takes data from time k+1
    ...
    if noPred % Don't include prediction
        ...
        P_kp1Minus = F_k*P_kPlus*F_k' + Q_k; %Omega_k*Q_k*Omega_k';
    end
    ...
    if noMeas % Don't correct if measurements are turned off
        ...
        for kk = 1:size(y{k+1},2) % Account for varying p at each timestep
            ...
            yNom = [yNom; rho_stat(x_nom, x_s, y_nom, y_s); rhoDot_stat(x_nom, x_s, xDot_nom, xDot_s, y_nom, y_s, yDot_nom, yDot_s); phi_stat(x_nom, x_s, y_nom, y_s)];
