...
Fcl = F - Khat;
correct = false;
E = zeros(length(Y), size(F,1));
E_correct = E;
R = zeros(length(Y));
Uhat = zeros(length(Y), size(F,1));
for k = 1:length(Y)
...
%     if ~correct
        % Build E
        elem = H*(F^kMath);
        rowIdx = size(elem,1)*(k-1)+1:size(elem,1)*(k);
        E(rowIdx,:) = elem;
    
        % Build R
        block = zeros(size(H,1),length(Y)); % Reset helper variable for building R
        for kk = k:-1:1 % Build up the block from left to right
            kkMath = kk - 1; % Create index for math that starts at 0 for implementation
            colIdx = size(G,2)*(kk-1)+1:size(G,2)*(kk);
            if kk == 1
                block(1:size(H,1), k-kk+1) = M;
            else
                block(1:size(H,1), k-kk+1) = H*(F^(kkMath-1))*G;
            end
        end
...
if ~correct
    numMeas = 260; % When numMeas < 260ish, good results..........
    L = L_orig(1:numMeas,:);
    dx0 = (((L'*L)^-1)*L')*Y(1:numMeas)
else
    L = L_corr;
