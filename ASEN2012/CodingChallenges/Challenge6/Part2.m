...
% copy-paste your code from part 1 here
...
V = (4/3)*pi*r0^3; % volume [m3]
Ac = pi*r0^2; % cross-sectional area [m2]
...
const = [r0, g0, CD, rho_air, rho_gas, V, Ac, m_sys]; % Vector or structure containing important geometric and constant values. Note the index (if using a vector) ...
...
figure(); hold on
    ...
    plot(t,v)
    title("Vertical Ascent Velocity of a High-Altitude Weather Balloon, Unvaried Parameters");
    xlabel("Time (sec)");
...
r = r0;
vMax = 25;
vterm = 1.5*vMax; %Arbitrary starting condition
% add variation
while (vterm > vMax) %Decrease r in 1 m increments, then see the result on terminal velocity and keep repeating until the terminal velocity reaches an acceptable level
    r = r - 1;
    ...
    [t,v] = ode45(@(t,v)calcAccel(t,v,const),tspan,IC); % Using the syntax required for ODE45 call the function here. 
    plot(t,v)
    title("Vertical Ascent Velocity of a High-Altitude Weather Balloon, radius of " + num2str(r) + " m");
    xlabel("Time (sec)");
    ylabel("Vertical velocity (m/s)");
    legend("Vertical velocity as computed by ODE45",'Location','best');
    vterm = max(v)
...
%% ------------------------ submission comment ------------------------- %%
%   All I would have to do to achieve this design is make a balloon with
%   a radius that is no bigger than the calculated "acceptable" radius. In
%   this case, that's about 14 m!
%% --------------------------------------------------------------------- %%

%% Functions

% copy-paste your odeFUNC code here
function [a] = calcAccel(t,v,const)
    ...
    %
    % INPUTS: Current time (solely for ODE45), current velocity, vector of constant conditions
    %  
    %
    % OUTPUTS: Acceleration at a given moment in time
    % 
