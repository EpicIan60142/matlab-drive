...
%% Propagate orbits and attitude with sun-pointing control
out_LMO = RK4_Orbit(x_0_LMO, t0, dt, tf);
out_GMO = RK4_Orbit(x_0_GMO, t0, dt, tf);

out_Attitude = RK4_Attitude(x_0_att, t0, dt, tf, 'sun pointing', out_LMO, out_GMO, controlParams);
...
f1 = fopen("gains.txt", "w");
ans_gains = fprintf(f1, "%.4f %.4f", P, K);
fclose(f1);

f2 = fopen("sig15_ans.txt", "w");
ans_sig15 = fprintf(f2, "%.3f %.3f %.3f", sig15(1), sig15(2), sig15(3));
fclose(f2);

f3 = fopen("sig100_ans.txt", "w");
ans_sig100 = fprintf(f3, "%.3f %.3f %.3f", sig100(1), sig100(2), sig100(3));
fclose(f3);

f4 = fopen("sig200_ans.txt", "w");
ans_sig200 = fprintf(f4, "%.3f %.3f %.3f", sig200(1), sig200(2), sig200(3));
fclose(f4);

f5 = fopen("sig400_ans.txt", "w");
ans_sig400 = fprintf(f5, "%.3f %.3f %.3f", sig400(1), sig400(2), sig400(3));
fclose(f5);
