...
% Group 11, 10:40 lab
%
% Code by Ian Faber and Daniel Mascarenas
...
for k = 2:8*(maxDim/2) % 1/4 in to 6 in diameter
    ...
    if(factorCircle >= 1.5 && factorCircle <= 1.53)
        ...
        factors.efficiency = [factors.efficiency; factorCircle/area];
    end
...
factors.efficiency = [factors.efficiency; 0];
...
for k = 1:4*maxDim % 1/4 in to 12 in length
    for l = 1:4*maxDim % 1/4 in to 12 in height
        ...
        sigmaBend = bendingStress(moment(area, 0), h/2, inertia);
        ...
        if(factorRectangle >= 1.5 && factorRectangle <= 1.53)
            ...
            factors.efficiency = [factors.efficiency; factorRectangle/area];
        end
    end
end
    ...
    for l = 1:4*maxDim % 1/4 in to 12 in height
        for m = 1:4*(maxDim/2) % 1/4 in to 6 in wall thickness
            ...
            if(factorHollow >= 1.5 && factorHollow <= 1.53)
                ...
                factors.efficiency = [factors.efficiency; factorHollow/area];
            end
        end
    end
end
    ...
    for l = 1:4*maxDim % 1/4 in to 12 in height
        for m = 1:4*maxDim % 1/4 in to 12 in middle thickness
            for n = 1:4*(maxDim/2) % 1/4 in to 6 in flange thickness
                ...
                inertia = ((h2*h1^3)/12)+((b1*(h1+2*h2)^3-h1)/12);
                ...
                if(factorIBeam >= 1.5 && factorIBeam <= 1.53)
                    ...
                    factors.efficiency = [factors.efficiency; factorIBeam/area];
                end
            end
        end
    end
    ...
    for l = 1:4*maxDim % 1/4 in to 12 in height
        for m = 1:4*maxDim % 1/4 in to 12 in middle thickness
            for n = 1:4*maxDim % 1/4 in to 12 in flange thickness
                ...
                centroid = (b1*b2*h1 +(1/2)*(b1*b2^2+b2*h1*h1^2))/(b1*b2+h1*b2);
                
                inertia = (1/3)*(b2*centroid^3+b1*(h1+b1-centroid)^3-(h1-b2)*(h1-centroid)^3);
                
                x = maxX(area);
                
                sigmaBend = bendingStress(moment(area, 0), centroid, inertia);
                ...
                if(factorTBeam >= 1.5 && factorTBeam <= 1.53)
                    ...
                    factors.efficiency = [factors.efficiency; factorTBeam/area];
                end
...
circle = struct('shape',factors.shape(1:(shapeChange(1)-1)),'area',factors.area(1:(shapeChange(1)-1)),'FoS',factors.factorOfSafety(1:(shapeChange(1)-1)),'efficiency',factors.efficiency(1:(shapeChange(1)-1)));
rectangle = struct('shape',factors.shape((shapeChange(1)+1):(shapeChange(2)-1)),'area',factors.area((shapeChange(1)+1):(shapeChange(2)-1)),'FoS',factors.factorOfSafety((shapeChange(1)+1):(shapeChange(2)-1)),'efficiency',factors.efficiency((shapeChange(1)+1):(shapeChange(2)-1)));
hollow = struct('shape',factors.shape((shapeChange(2)+1):(shapeChange(3)-1)),'area',factors.area((shapeChange(2)+1):(shapeChange(3)-1)),'FoS',factors.factorOfSafety((shapeChange(2)+1):(shapeChange(3)-1)),'efficiency',factors.efficiency((shapeChange(2)+1):(shapeChange(3)-1)));
IBeam = struct('shape',factors.shape((shapeChange(3)+1):(shapeChange(4)-1)),'area',factors.area((shapeChange(3)+1):(shapeChange(4)-1)),'FoS',factors.factorOfSafety((shapeChange(3)+1):(shapeChange(4)-1)),'efficiency',factors.efficiency((shapeChange(3)+1):(shapeChange(4)-1)));
TBeam = struct('shape',factors.shape((shapeChange(4)+1):end),'area',factors.area((shapeChange(4)+1):end),'FoS',factors.factorOfSafety((shapeChange(4)+1):end),'efficiency',factors.efficiency((shapeChange(4)+1):end));
