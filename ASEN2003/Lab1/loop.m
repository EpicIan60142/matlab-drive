...
function theta_in = loop_starting_angle(vel_in)
   ...
   if sign(vel_in(2)) > 0 % Positive initial x-velocity means the loop is counterclockwise
...
function [path, distance, theta_vec] = loop_compute_path(pos_in, theta_in, vel_in, n, prop, r)
   ...
   if sign(vel_in(2)) > 0 % Positive initial x-velocity means the loop is counterclockwise
       % TO DO: complete this section of code 
       path = pos_in + [3*(theta_vec'-theta_in), -r*sin(theta_in) + r*cos(theta_vec' - pi/2), r*cos(theta_in) + r*sin(theta_vec' - pi/2)]; %[x_pos, y_pos, z_pos]
   else % Otherwise (negative initial x-velocity), the loop is clockwise
       % TO DO: complete this section of code
       path = pos_in + [3*(theta_vec'-theta_in), r*sin(theta_in) - r*cos(theta_vec' - pi/2), r*cos(theta_in) + r*sin(theta_vec' - pi/2)]; %[x_pos, y_pos, z_pos]
   end
   ...
   distance = cumtrapz(theta_vec, sqrt(3^2 + (r*sin(theta_vec'-pi/2)).^2 + (r*cos(theta_vec'-pi/2)).^2));
...
function vel_out = loop_exit_vel(path, theta_vec, h0, vel_in)
    ...
    vel_out = [3, (mag-sqrt(3))*cos(2*pi - theta_vec(end)), (mag-sqrt(3))*sin(2*pi - theta_vec(end))]; %[x_vel, y_vel, z_vel]
end
