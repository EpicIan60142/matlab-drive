...
for i = 0:5
 h = 2*i+1;
 g = 2*i+2;
[P1,P2] = Chris(9.5,0.5,B(length(B),2),0.01*i);
Chrismat2(:,h)= P1;
Chrismat2(:,g)= P2;
end 
...
h = 2*i+1;
    g = 2*i+2;
[P1,P2] = Chris(9,-2.5,C(length(C),2),0.01*i);
Chrismat3(:,h)= P1;
Chrismat3(:,g)= P2;
end 
...
for i = 0:5
    h = 2*i+1;
    g = 2*i+2;
[P1,P2] = Chris(6,0.5,D(length(D),2),0.01*i);
Chrismat4(:,h)= P1;
Chrismat4(:,g)= P2;
end 
...
title('Bode plot for time vs angular position with Kp = 8 and Kd = 0.01')
xlim([0 4])
hold off
...
xlabel('Time (sec)')
ylabel('Angular Position (rad)')
legend('Experimental results','B=0','B=0.01','B=0.02','B=0.03','B=0.04','B=0.05','Location','best')
title('Bode plot for time vs angular position with Kp = 6.99 and Kd = -0.121')
xlim([0 4])
hold off

figure(7)%based on kp =6 kd = 0.5
plot(exp(:,3)-exp(startB,3),exp(:,4))
hold on 
